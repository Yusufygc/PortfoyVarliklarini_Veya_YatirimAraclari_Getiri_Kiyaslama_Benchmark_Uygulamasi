...
def load_cpi_series(filepath: str, cache_path: str = "data", auto_refresh: bool = True) -> pd.Series:
    """Aylık CPI endeks CSV'sini günlük seriye genişlet.

    auto_refresh=True ve TTL dolmuşsa, TCMB Inflation Data scraper'ı tetikler
    (MoM% compound ile mevcut CSV'yi son aya kadar günceller). Scrape başarısız
    olursa graceful: mevcut CSV kullanılır.
    """
...
def load_tcmb_rates(
    ...
    """
    Tier 1: TTL dolmuşsa TCMB 1-Week Repo scraper'ını tetikler.
